...
class HubSpotService {
  ...
  async getContacts(limit = 100): Promise<HubSpotContact[]> {
    ...
    while (hasMore) {
      ...
      if (data.results && data.results.length > 0) {
        ...
        if (allContacts.length === data.results.length) {
          console.log(
            'First contact properties:',
      ...
      hasMore =
        data.paging && data.paging.next && data.paging.next.after;
  ...
  async getCompanies(limit = 100): Promise<HubSpotCompany[]> {
    ...
    while (hasMore) {
      ...
      if (data.results && data.results.length > 0) {
        ...
        if (allCompanies.length === data.results.length) {
          console.log(
            'First company properties:',
            JSON.stringify(data.results[0].properties, null, 2)
          );
        }
      }
  ...
  async countAllDeals(): Promise<number> {
    ...
    while (hasMore) {
      console.log(`Counting deals page...`);
      ...
      console.log(
        `Page deals: ${dealsInPage}, Total so far: ${totalCount}`
      );
  ...
  async getDeals(limit = 100): Promise<HubSpotDeal[]> {
    ...
    while (hasMore) {
      ...
      if (data.results && data.results.length > 0) {
        ...
        console.log(
          `Page deals added. Total deals so far: ${allDeals.length}`
        );

        if (allDeals.length === data.results.length) {
          console.log(
            'First deal properties:',
            JSON.stringify(data.results[0].properties, null, 2)
          );
        }

        const pageStages = data.results.map(
          (d: any) => d.properties.dealstage
        );
        console.log(`Page deal stages:`, pageStages);
      }
      ...
      console.log(`Page has more pages: ${hasMore}`);
      if (hasMore) {
        console.log(`Next page cursor: ${after}`);
      }

      // Only stop if we've reached the requested limit
      if (limit > 0 && allDeals.length >= limit) {
        console.log(
          `Reached limit of ${limit} deals, stopping pagination`
        );
        break;
      }
    }
    ...
    const finalStages = Array.from(
      new Set(allDeals.map((deal) => deal.properties.dealstage))
    );
  ...
  async getTasks(limit = 100): Promise<HubSpotTask[]> {
    ...
    while (hasMore) {
      ...
      if (data.results && data.results.length > 0) {
        ...
        if (allTasks.length === data.results.length) {
          console.log(
    ...
    try {
      ...
      if (deals.length > 0) {
        ...
        const dealStages = Array.from(
          new Set(deals.map((deal) => deal.properties.dealstage))
        );
        ...
        deals.slice(0, 5).forEach((deal, index) => {
          console.log(
            `Deal ${index + 1}: ${deal.properties.dealname} - Stage: ${
